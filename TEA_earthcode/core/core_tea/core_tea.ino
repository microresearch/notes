...
- test primitive DAC out on PF0-PF7 (MSB)

- maybe some problem with enables as only diagonal cores work but maybe we can simulate this (with bit rot)

eg. read in ADC to cores (8 bits->each line), bitrot of random bits, out to DAC PORTF(each line)

- remove ISR-DONE

we need to connect logic 5v and GND

- test basic functionality of core reads and writes
- serial out
- what we output - analogue ladder to magnetometer?
...
inline byte pulserl(byte x, byte y){
  ...
  byte smpl = PINJ & 0x01; // sense is on 14 = PJ1 - check if is now 15? was 0x02
  __asm__ ("nop\n\tnop\n\tnop\n\tnop\n\tnop\n\tnop\n\t");  // 
  __asm__ ("nop\n\tnop\n\tnop\n\tnop\n\tnop\n\tnop\n\t");  //
...
inline byte pulserh(byte x, byte y){
  ...
  // sample sense amp output after ~1,200 ns delay
  byte smpl = PINJ & 0x01; // sense is on 14 = PJ1 - checked  15 is PJ0
  __asm__ ("nop\n\tnop\n\tnop\n\tnop\n\tnop\n\tnop\n\t");  // 
  __asm__ ("nop\n\tnop\n\tnop\n\tnop\n\tnop\n\tnop\n\t");  //
...
void setup() {
  ...
  byte corex,corey;
    ...
    void loop() {
      ...
      // test code for ADC8 read and output to DAC:

      uint16_t readerbyte;
      byte x, y, corex, corey;

      //      readerbyte=analogRead(8);
      //      PORTF=readerbyte; // output

      // fake core code
      // 8 bytes = 64 bits
      for (y=0;y<8;y++){
	PORTF=cory[y];
	cory[y]=analogRead(8)&255;
      }
      // flip random bit
      corex=rand()%8;
      corey=rand()%8;
      if ((rand()%32)==0) cory[corex]^=(1<<corey);
      ...
      for (y=0;y<8;y++){
      for (x=0;x<8;x++){
      delay(500); // every 2 seconds flip the first bit
      posx=x; posy=y;
