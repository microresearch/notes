...
// transforming into DIANA interpreter/lunar code

- bit by bit entry from pin 4 which is mapped to: PD4 in interrupt 
...
unsigned char offsetdi; // 0-255 is our time offset for sun and moon calc and scale these
double RA, Dec,  topRA, topDec,  LST, HA, Az, El, dist;
float timedi, timeoff=0.0f;
unsigned char countx=0;
unsigned char instructionpX[255];
unsigned char howmanyX=0;
u8 UNK;
...
unsigned int dodisun(int offset) {
 ...
 JD_frac=(hour+minute/60.+second/3683.3)/24.-.5; // adjusted for our 255 time
 ...
 unsigned int scaled=(int)(azimuth*elev*1000.0);
 u8 scale=scaled%255;
 return scale;
}
...
void AddManBit(uint16_t *manBits, uint8_t *numMB, uint8_t bit)
{
  ...
  *manBits |= bit;
  ...
  if (*numMB == 8)
  {
    ...
    //    xxx[countx++]=newData;
    //    Serial.write(*manBits);
    *numMB = 0;
  }
}

void AddManBitX(uint16_t *manBits, uint8_t *numMB, uint8_t bit)
{
  *manBits <<= 1;
  *manBits |= bit;
  (*numMB)++;
...
void initcell(unsigned char* cells){
  unsigned int x;
  for (x=0;x<MAX_SAM*8;x++){
    uint8_t rx_sample_1=0;
    rx_sample_1 = (PIND&(1<<4))>>4;
    AddManBitX(&rx_manBits, &rx_numMB, rx_sample_1); // read into UNK and cells
    delay(1);
    }  
}

void initcellX(unsigned char* cells){
  unsigned char x;
  for (x=0;x<MAX_SAM;x++){
    //    cells[x]=randi();
...
unsigned char diroutX(unsigned char* cells, unsigned int IP){
  modifier=cells[IP];
  unsigned char modout=modifier%95;
  modout+=32;
  Serial.write(modout);
  return (IP+3)%MAX_SAM;
}

// read into OPCODE, A or B from UNK - can also write to memory locations - direct and indirect from B
unsigned char redinop_imm(unsigned char* cells, unsigned int IP){ 
  cells[IP]=UNK; // inst
  return (IP+3)%MAX_SAM;
}

unsigned char redinopA_imm(unsigned char* cells, unsigned int IP){ 
  cells[(IP+1)%MAX_SAM]=UNK; // inst
  return (IP+3)%MAX_SAM;
}

unsigned char redinopB_imm(unsigned char* cells, unsigned int IP){ 
  cells[(IP+2)%MAX_SAM]=UNK; // inst
  return (IP+3)%MAX_SAM;
}

// read in to direct
unsigned char redinop_dir(unsigned char* cells, unsigned int IP){
  unsigned int A=cells[(IP+1)%MAX_SAM];
  unsigned int B=cells[(IP+2)%MAX_SAM];
  unsigned int tmpp=(IP+B);  // mem[(addr + inst.b) % MEMSIZE] = data;
  cells[tmpp%MAX_SAM]=UNK; // inst
  return (IP+3)%MAX_SAM;
}

unsigned char redinopA_dir(unsigned char* cells, unsigned int IP){
  unsigned int A=cells[(IP+1)%MAX_SAM];
  unsigned int B=cells[(IP+2)%MAX_SAM];
  unsigned int tmpp=(IP+B);  // mem[(addr + inst.b) % MEMSIZE] = data;
  cells[(tmpp+1)%MAX_SAM]=UNK; // inst
  return (IP+3)%MAX_SAM;
}

unsigned char redinopB_dir(unsigned char* cells, unsigned int IP){
  unsigned int A=cells[(IP+1)%MAX_SAM];
  unsigned int B=cells[(IP+2)%MAX_SAM];
  unsigned int tmpp=(IP+B);  // mem[(addr + inst.b) % MEMSIZE] = data;
  cells[(tmpp+2)%MAX_SAM]=UNK; // inst
  return (IP+3)%MAX_SAM;
}

// read in to indirect
unsigned char redinop_indir(unsigned char* cells, unsigned int IP){
  unsigned int A=cells[(IP+1)%MAX_SAM];
  unsigned int B=cells[(IP+2)%MAX_SAM];
  unsigned int tmpp=(IP+B);  // mem[(addr + inst.b) % MEMSIZE] = data;
  unsigned int dest=cells[(tmpp+2)%MAX_SAM];  // mem[(destination + addr + inst.b) % MEMSIZE] = data;
  cells[dest%MAX_SAM]=UNK; // inst
  return (IP+3)%MAX_SAM;
}

unsigned char redinopA_indir(unsigned char* cells, unsigned int IP){
  unsigned int A=cells[(IP+1)%MAX_SAM];
  unsigned int B=cells[(IP+2)%MAX_SAM];
  unsigned int tmpp=(IP+B);  // mem[(addr + inst.b) % MEMSIZE] = data;
  unsigned int dest=cells[(tmpp+2)%MAX_SAM];  // mem[(destination + addr + inst.b) % MEMSIZE] = data;
  cells[(dest+1)%MAX_SAM]=UNK; // inst
  return (IP+3)%MAX_SAM;
}

unsigned char redinopB_indir(unsigned char* cells, unsigned int IP){
  unsigned int A=cells[(IP+1)%MAX_SAM];
  unsigned int B=cells[(IP+2)%MAX_SAM];
  unsigned int tmpp=(IP+B);  // mem[(addr + inst.b) % MEMSIZE] = data;
  unsigned int dest=cells[(tmpp+2)%MAX_SAM];  // mem[(destination + addr + inst.b) % MEMSIZE] = data;
  cells[(dest+2)%MAX_SAM]=UNK; // inst
  return (IP+3)%MAX_SAM;
}

// 

// DIANAintructions/// eg. paparazzi

/*
??PI - pillar (bounce back to address or stop/halt=reset time counter-we have reset)
??FI - fiat - strike fiat and deviate course - how differs from pillars or hp
??MC - mercedes - vehicle??? propels //
*/


//DO - dodi - sungod . sun position? - translate all moon instructions
/*
  unsigned char solar=dodisun(offsetdi);
  cells[(tmpp+2)%MAX_SAM]=solar; // B   //  			data.b = inst.a;
 */


// 
unsigned char dianaHP(unsigned char* cells, unsigned int IP){
  u8 tmp=UNK+IP;
  if (tmp>0)
    return tmp; 
 else
...
unsigned char dianaRDop(unsigned char* cells, unsigned int IP){ 
  unsigned int A=cells[(IP+1)%MAX_SAM];
  unsigned int B=cells[(IP+2)%MAX_SAM];
  u8 amount=B&0x08;
  u8 tmp= (IP >> amount) | ( IP << (8 - amount));
  cells[IP]=tmp;
  return (IP+3)%MAX_SAM;
}

unsigned char dianaRDA(unsigned char* cells, unsigned int IP){ 
  unsigned int A=cells[(IP+1)%MAX_SAM];
  unsigned int B=cells[(IP+2)%MAX_SAM];
  u8 amount=B&0x08;
  u8 tmp= (A >> amount) | ( A << (8 - amount));
  cells[(IP+1)%MAX_SAM]=tmp;
  return (IP+3)%MAX_SAM;
}

unsigned char dianaRDB(unsigned char* cells, unsigned int IP){ 
  unsigned int A=cells[(IP+1)%MAX_SAM];
  unsigned int B=cells[(IP+2)%MAX_SAM];
  u8 amount=A&0x08;
  u8 tmp= (B >> amount) | (B << (8 - amount));
  cells[(IP+2)%MAX_SAM]=tmp;
  return (IP+3)%MAX_SAM;
}

unsigned char dianaRDdir(unsigned char* cells, unsigned int IP){  // rotate mem address at B by A
  unsigned int A=cells[(IP+1)%MAX_SAM];
  unsigned int B=cells[(IP+2)%MAX_SAM];
  unsigned int tmpp=(IP+B);  // mem[(addr + inst.b) % MEMSIZE] = data;
  u8 amount=A&0x08;
  u8 tmp= (cells[(tmpp+2)%MAX_SAM] >> amount) | (cells[(tmpp+2)%MAX_SAM] << (8 - amount));
  cells[(tmpp+2)%MAX_SAM]=tmp;
  return (IP+3)%MAX_SAM;
}

unsigned char dianaRDindir(unsigned char* cells, unsigned int IP){ 
  unsigned int A=cells[(IP+1)%MAX_SAM];
  unsigned int B=cells[(IP+2)%MAX_SAM];
  u8 amount=B&0x08;
  unsigned int src=cells[(IP+A+2)%MAX_SAM]; // B
  u8 tmp= (cells[(src+IP+A+1)%MAX_SAM] >> amount) | (cells[(src+IP+A+1)%MAX_SAM] << (8 - amount));
  cells[(src+IP+A+1)%MAX_SAM]=tmp;
  return (IP+3)%MAX_SAM;
}

// test fork
unsigned char redSPL(unsigned char* cells, unsigned int IP){
  howmanyX++;
  unsigned int B=cells[(IP+2)%MAX_SAM];
  instructionpX[howmanyX]=(IP+B)%MAX_SAM; // what is it ... B
  return (IP+3)%MAX_SAM;
}

unsigned char dianaPA_dir(unsigned char* cells, unsigned int IP){ // PA - paparazzi eg. show next x bytes of memory... - direct...
  // Serial write an array: 
  unsigned char arr[MAX_SAM];
  unsigned int A=cells[(IP+1)%MAX_SAM];
  unsigned int B=cells[(IP+2)%MAX_SAM];
  // A is offset, B is amount
  for (unsigned char x=0;x<B;x++){
    modifier=cells[(IP+A+x)%MAX_SAM];
    unsigned char modout=modifier%95;
    modout+=32;
    arr[x]=modout;
  }
...
unsigned char dianaPA_indir(unsigned char* cells, unsigned int IP){ // PA - paparazzi eg. show next x bytes of memory... - direct...
  // Serial write an array: 
  unsigned char arr[MAX_SAM];
  unsigned int A=cells[(IP+1)%MAX_SAM];
  unsigned int B=cells[(IP+2)%MAX_SAM];
  ...
  unsigned int src=cells[(IP+A+2)%MAX_SAM]; // B
  for (unsigned char x=0;x<B;x++){
    modifier=cells[(IP+src+x)%MAX_SAM];
    unsigned char modout=modifier%95;
    modout+=32;
    arr[x]=modout;
  }
...
// place moon value at B//indirect etc... see how it goes - start simple
unsigned char dianaDImoon_realtodir(unsigned char* cells, unsigned int IP){ // unused A
  unsigned int A=cells[(IP+1)%MAX_SAM];
  unsigned int B=cells[(IP+2)%MAX_SAM];
  unsigned int tmpp=(IP+B);  // mem[(addr + inst.b) % MEMSIZE] = data;
  // offsetdi
  timedi=(22+20/60.0) + (float)offsetdi/3683.3;
  moon2(1997, 8, 30, timedi, 2.301889, 48.864361,
  ...
  // how do we scale it to 8 bit 0-255 as it crosses 360 degrees... - last digits...
  unsigned int scaled=(int)(Az*El*1000.0);
  u8 lunar=scaled%255;
  cells[(tmpp+2)%MAX_SAM]=lunar; // B   //  			data.b = inst.a;
  //  Serial.println(lunar);
  return (IP+3)%MAX_SAM;
}

unsigned char dianaDImoon_realtoindir(unsigned char* cells, unsigned int IP){
  unsigned int A=cells[(IP+1)%MAX_SAM];
  unsigned int B=cells[(IP+2)%MAX_SAM];
  unsigned int tmpp=(IP+B);  // mem[(addr + inst.b) % MEMSIZE] = data;
  // offsetdi
  ...
  u8 lunar=scaled%255;
  unsigned int dest=cells[(tmpp+2)%MAX_SAM];  // mem[(destination + addr + inst.b) % MEMSIZE] = data;
  cells[(dest+IP+B+2)%MAX_SAM]=lunar;
  //  Serial.println(lunar);
  return (IP+3)%MAX_SAM;
}

/*
DI get moon value at real time we have as offsetdi DONE
get moon value at time x (x from memory, absolute value=byte)DONE

DONE: jump to moon value (sets instruction pointer)
cmp moon value and branch???maybeTODO

- and DONEtest for sun now below// repeat sun as moon

*/

unsigned char dianaDImoon_realjmpdir(unsigned char* cells, unsigned int IP){ 
  // offsetdi
  timedi=(22+20/60.0) + (float)offsetdi/3683.3;
  moon2(1997, 8, 30, timedi, 2.301889, 48.864361,
	&RA, &Dec, &topRA, &topDec, &LST, &HA, &Az, &El, &dist); // +02:00 GMT (DST) Thu 12:46 pm CEST - so we need to subtract 2 hours to get correct calculation
  ...
  unsigned int scaled=(int)(Az*El*1000.0);
  u8 lunar=scaled%255;
  u8 tmp=(IP+lunar)%MAX_SAM;
  if (tmp>0) return tmp;
  return (IP+3)%MAX_SAM;
}

unsigned char dianaDImoon_realjmpindir(unsigned char* cells, unsigned int IP){ 
  // offsetdi
  timedi=(22+20/60.0) + (float)offsetdi/3683.3;
  moon2(1997, 8, 30, timedi, 2.301889, 48.864361,
	&RA, &Dec, &topRA, &topDec, &LST, &HA, &Az, &El, &dist); // +02:00 GMT (DST) Thu 12:46 pm CEST - so we need to subtract 2 hours to get correct calculation
  ...
  unsigned int scaled=(int)(Az*El*1000.0);
  u8 lunar=scaled%255;
  u8 tmp=cells[(IP+lunar)%MAX_SAM];
  if (tmp>0) return tmp;
  else return (IP+3)%MAX_SAM;
}

unsigned char dianaDImoon_drealjmpdir(unsigned char* cells, unsigned int IP){ 
  // offsetdi
  unsigned int A=cells[(IP+1)%MAX_SAM];
  unsigned char off=(offsetdi+A);
  timedi=(22+20/60.0) + (float)off/3683.3;
  moon2(1997, 8, 30, timedi, 2.301889, 48.864361,
	&RA, &Dec, &topRA, &topDec, &LST, &HA, &Az, &El, &dist); // +02:00 GMT (DST) Thu 12:46 pm CEST - so we need to subtract 2 hours to get correct calculation
  // how do we scale it to 8 bit 0-255 as it crosses 360 degrees... - last digits...
  unsigned int scaled=(int)(Az*El*1000.0);
...
unsigned char dianaDImoon_drealjmpindir(unsigned char* cells, unsigned int IP){ 
  // offsetdi
  unsigned int A=cells[(IP+1)%MAX_SAM];
  unsigned char off=(offsetdi+A);
  timedi=(22+20/60.0) + (float)off/3683.3;
...
unsigned char dianaDImoon_drealtodir(unsigned char* cells, unsigned int IP){ // unused A
  unsigned int A=cells[(IP+1)%MAX_SAM];
  unsigned int B=cells[(IP+2)%MAX_SAM];
  unsigned int tmpp=(IP+B);  // mem[(addr + inst.b) % MEMSIZE] = data;
  unsigned char off=(offsetdi+A);
...
unsigned char dianaDImoon_drealtoindir(unsigned char* cells, unsigned int IP){
  unsigned int A=cells[(IP+1)%MAX_SAM];
  unsigned int B=cells[(IP+2)%MAX_SAM];
  unsigned int tmpp=(IP+B);  // mem[(addr + inst.b) % MEMSIZE] = data;
  // offsetdi
...
// sun tested but we need to repeat all above with sun
unsigned char dianaDODIsun_realtodir(unsigned char* cells, unsigned int IP){ // unused A
  unsigned int A=cells[(IP+1)%MAX_SAM];
  unsigned int B=cells[(IP+2)%MAX_SAM];
  unsigned int tmpp=(IP+B);  // mem[(addr + inst.b) % MEMSIZE] = data;
  // offsetdi
...
unsigned char dianaDODIsun_realtoindir(unsigned char* cells, unsigned int IP){
  unsigned int A=cells[(IP+1)%MAX_SAM];
  unsigned int B=cells[(IP+2)%MAX_SAM];
  unsigned int tmpp=(IP+B);  // mem[(addr + inst.b) % MEMSIZE] = data;
  unsigned int dest=cells[(tmpp+2)%MAX_SAM];  // mem[(destination + addr + inst.b) % MEMSIZE] = data;
  unsigned char solar=dodisun(offsetdi);
  cells[(dest+IP+B+2)%MAX_SAM]=solar;
  //  Serial.println(lunar);
  return (IP+3)%MAX_SAM;
}

unsigned char dianaDODIsun_realjmpdir(unsigned char* cells, unsigned int IP){ 
  // offsetdi
  unsigned char solar=dodisun(offsetdi);
  u8 tmp=(IP+solar)%MAX_SAM;
  if (tmp>0) return tmp;
  return (IP+3)%MAX_SAM;
}

unsigned char dianaDODIsun_realjmpindir(unsigned char* cells, unsigned int IP){ 
  // offsetdi
  unsigned char solar=dodisun(offsetdi);
  u8 tmp=cells[(IP+solar)%MAX_SAM];
  if (tmp>0) return tmp;
  else return (IP+3)%MAX_SAM;
}

unsigned char dianaDODIsun_drealjmpdir(unsigned char* cells, unsigned int IP){ 
  // offsetdi
  unsigned int A=cells[(IP+1)%MAX_SAM];
  unsigned char off=(offsetdi+A);
  unsigned char solar=dodisun(off);
  u8 tmp=(IP+solar)%MAX_SAM;
  if (tmp>0) return tmp;
  else return (IP+3)%MAX_SAM;
}

unsigned char dianaDODIsun_drealjmpindir(unsigned char* cells, unsigned int IP){ 
  // offsetdi
  unsigned int A=cells[(IP+1)%MAX_SAM];
  unsigned char off=(offsetdi+A);
  unsigned char solar=dodisun(off);
...
unsigned char dianaDODIsun_drealtodir(unsigned char* cells, unsigned int IP){ // unused A
  unsigned int A=cells[(IP+1)%MAX_SAM];
  unsigned int B=cells[(IP+2)%MAX_SAM];
  unsigned int tmpp=(IP+B);  // mem[(addr + inst.b) % MEMSIZE] = data;
  unsigned char off=(offsetdi+A);
...
unsigned char dianaDODIsun_drealtoindir(unsigned char* cells, unsigned int IP){
  unsigned int A=cells[(IP+1)%MAX_SAM];
  unsigned int B=cells[(IP+2)%MAX_SAM];
  unsigned int tmpp=(IP+B);  // mem[(addr + inst.b) % MEMSIZE] = data;
  // offsetdi
...
////////////////////////////
/*
TIME instructions: offsetdi
- set time relative from memory - *forward, backwards* // 
x- set time to instruction pointer
x- set instruction pointer to time...
x- set time to 0 // start again
*/
////////////////////////////
unsigned char dianaTIMEreset(unsigned char* cells, unsigned int IP){ 
  offsetdi=0; // or does each process have its own time?
  return (IP+3)%MAX_SAM;
}

unsigned char dianaTIMEtoIP(unsigned char* cells, unsigned int IP){ 
  offsetdi=IP; // or does each process have its own time?
  return (IP+3)%MAX_SAM;
}

unsigned char dianaIPtoTIME(unsigned char* cells, unsigned int IP){ 
  if (offsetdi>0) return offsetdi;
  else return (IP+3)%MAX_SAM;
}

unsigned char dianaTIMEimmfwd(unsigned char* cells, unsigned int IP){
  unsigned int A=cells[(IP+1)%MAX_SAM];
  unsigned int B=cells[(IP+2)%MAX_SAM];
  offsetdi=(IP+B)%255;  // mem[(addr + inst.b) % MEMSIZE] = data;
  return (IP+3)%MAX_SAM;
}

unsigned char dianaTIMEimmback(unsigned char* cells, unsigned int IP){
  unsigned int A=cells[(IP+1)%MAX_SAM];
  unsigned int B=cells[(IP+2)%MAX_SAM];
  offsetdi=(IP-B)%255;  // mem[(addr + inst.b) % MEMSIZE] = data;
  return (IP+3)%MAX_SAM;
...
unsigned char dianaTIMEdirfwd(unsigned char* cells, unsigned int IP){
  unsigned int A=cells[(IP+1)%MAX_SAM];
  unsigned int B=cells[(IP+2)%MAX_SAM];
  offsetdi+=(cells[(IP+A)%MAX_SAM]); 
  return (IP+3)%MAX_SAM;
}

unsigned char dianaTIMEdirback(unsigned char* cells, unsigned int IP){
  unsigned int A=cells[(IP+1)%MAX_SAM];
  unsigned int B=cells[(IP+2)%MAX_SAM];
  offsetdi-=(cells[(IP+A)%MAX_SAM]);
  return (IP+3)%MAX_SAM;
...
unsigned char dianaTIMEindirfwd(unsigned char* cells, unsigned int IP){
  unsigned int A=cells[(IP+1)%MAX_SAM];
  unsigned int B=cells[(IP+2)%MAX_SAM];
  unsigned int src=cells[(IP+A+2)%MAX_SAM]; // B
  offsetdi+=cells[(src+IP+A+1)%MAX_SAM];  // A
  return (IP+3)%MAX_SAM;
}

unsigned char dianaTIMEindirback(unsigned char* cells, unsigned int IP){
  unsigned int A=cells[(IP+1)%MAX_SAM];
  unsigned int B=cells[(IP+2)%MAX_SAM];
  unsigned int src=cells[(IP+A+2)%MAX_SAM]; // B
  offsetdi-=cells[(src+IP+A+1)%MAX_SAM]; 
  return (IP+3)%MAX_SAM;
}
...
unsigned char redjmz(unsigned char* cells, unsigned int IP){
  unsigned int A=cells[(IP+1)%MAX_SAM];
  unsigned int B=cells[(IP+2)%MAX_SAM];  
  if (cells[(B%MAX_SAM)]==0 && cells[(B+2)%MAX_SAM]==0 && A!=0)
    return (IP+A)%MAX_SAM;
  else
      return (IP+3)%MAX_SAM;
}
...
ISR(TIMER2_COMPA_vect)
{
    uint8_t rx_sample_1=0;
    rx_sample_1 = (PIND&(1<<4))>>4;
    AddManBit(&rx_manBits, &rx_numMB, rx_sample_1); // read into UNK and cells
}
#endif
...
void setup(){
  ...
  unsigned char (*instructionsetredcode[])(unsigned char* cells, unsigned char IP) = {rdmov,rdadd,rdsub,rdjmp,rdjmz,rdjmg,rddjz,rddat,rdcmp,rdoutp,dirout}; // 10+1+1=12
...
unsigned char (*DIANOS[])(unsigned char* cells, unsigned int IP) = {dianaPA_dir, dianaPA_indir, redSPL, dianaDImoon_drealtodir, dianaDImoon_drealtoindir, dianaDImoon_realtodir, dianaDImoon_realtoindir, dianaDImoon_realjmpdir, dianaDImoon_realjmpindir, dianaDImoon_drealjmpdir, dianaDImoon_drealjmpindir, dianaDODIsun_drealtodir, dianaDODIsun_drealtoindir, dianaDODIsun_realtodir, dianaDODIsun_realtoindir, dianaDODIsun_realjmpdir, dianaDODIsun_realjmpindir, dianaDODIsun_drealjmpdir, dianaDODIsun_drealjmpindir, redmovimm_to_dir, redmovimm_to_indir, redmovdir_to_dir, redmovdir_to_indir, redmovindir_to_dir, redmovindir_to_indir, redaddimm_to_dir, redaddimm_to_indir, redadddir_to_dir, redadddir_to_indir, redaddindir_to_dir, redaddindir_to_indir, redsubimm_to_dir, redsubimm_to_indir, redsubdir_to_dir, redsubdir_to_indir, redsubindir_to_dir, redsubindir_to_indir, redjmp, redjmz, redjmg, reddjz, redcmpimm_to_dir, redcmpimm_to_indir, redcmpdir_to_dir, redcmpdir_to_indir, redcmpindir_to_dir, redcmpindir_to_indir, redinop_imm, redinopA_imm, redinopA_imm, redinop_dir, redinopA_dir, redinopA_dir, redinop_indir, redinopA_indir, redinopA_indir, dianaTIMEreset, dianaTIMEtoIP, dianaIPtoTIME, dianaTIMEimmfwd, dianaTIMEimmback, dianaTIMEdirfwd, dianaTIMEdirback, dianaTIMEdirfwd, dianaTIMEdirback, dianaRDop, dianaRDA, dianaRDB, dianaRDdir, dianaRDindir}; 
...
void loop(void)
{
  static unsigned char cpu, plague, countSPL=0;
#else
...
/*
the journey is from 00:20 to 00:23 according to paget...

UTC: say 22.20 to 22.23 30 aug 1997 - say 3 minutes = 180 // or shall we divide that into 255
...
*/
	...
	//        if ((counter%(1+cpuspeed))==0){ // ??? This is where we do speed!
	  ...
	  case 17: // do nothing
	    break;
	  case 18: // test cycling through array of instruction pointers for new redcode and SPL
	    countSPL++;
	    if (countSPL>howmanyX) countSPL=0;
	    instruction=cells[instructionpX[countSPL]];
	    instructionpX[countSPL]=(*DIANOS[instruction%DIANANO]) (cells, instructionpX[countSPL]); // mistake before as was instruction%INSTLEN in last instance
	  ...
	  case 16: // new redcode
	    ...
	    instructionp=(*DIANOS[instruction%DIANANO]) (cells, instructionp); // mistake before as was instruction%INSTLEN in last instance
	    //	    insdir=dir*(IP%16)+1; // prev mistake as just got exponentially larger
	    //	    printf("IP %d\n", instructionp);
	    insdir=dir;
	    break;
	  ...
	  case 4:
	    instruction=cells[instructionp];
	    instructionp=(*instructionsetredcode[instruction%11]) (cells, instructionp);  // was 12
	    insdir=dir; //NOt needed
	    break;
	  case 5:
  ...
  if (++counterr>=speed) { //once per sec test was passed
    ...
    */
    ...
    //    Serial.write(modout);
    //    Serial.println(xxx[oldone]);
    //    unsigned int testy=dodisun(255);
    //        Serial.print(instruction%DIANANO);
	//        Serial.println("runs");
#else
    //        printf("x: %d mod: %d\n",oldone, modifier);
        printf("%c",modifier);
