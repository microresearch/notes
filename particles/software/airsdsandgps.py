...
def main():
    ...
    # open file
    f = open("particlewalk_24_4_2020", "w")
        ...
        if record['sentence'] == 'RMC':
            ...
            if lastbyte == "\xAA" and byte == "\xC0":
                ...
                if record['longitude']!=0.0:
                    print record['time'],",", record['longitude'],",", record['latitude'],",",pm_25,",",pm_10
                    # and write to file.
                    f.write(str(record['time'])+","+str(record['longitude'])+","+ str(record['latitude'])+","+str(pm_25)+","+str(pm_10)+"\n")
    f.close()
